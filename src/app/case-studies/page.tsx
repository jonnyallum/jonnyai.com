...
function CaseStudyCard({ study, index }: { study: CaseStudy; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group glass-panel border-white/5 overflow-hidden hover:border-citrus/30 transition-all duration-500 hover:shadow-2xl hover:shadow-citrus/5"
    >
      {/* Visual Header */}
      <div className="aspect-video bg-obsidian-light relative overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          {study.featured && (
            <span className="bg-citrus/10 border border-citrus/30 text-citrus text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest backdrop-blur-md">
              Featured Mission
            </span>
          )}
      ...
      <div className="p-8">
        ...
        <h3 className="font-outfit font-black text-2xl text-white mb-4 group-hover:text-citrus transition-colors uppercase tracking-tight">
          {study.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {study.tech.slice(0, 4).map((tech) => (
            <span key={tech} className="text-[9px] font-black text-gray-400 bg-white/5 border border-white/10 px-3 py-1 rounded-lg uppercase tracking-wider group-hover:border-citrus/20 transition-all">
              {tech}
            </span>
          ))}
          {study.tech.length > 4 && (
            <span className="text-[9px] font-black text-citrus py-1 uppercase tracking-widest">+{study.tech.length - 4} Assets</span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-white/5">
          <Link
            ...
            className="flex items-center gap-2 text-citrus hover:text-white text-xs font-black uppercase tracking-widest transition-all"
          >
            Mission Debrief
  ...
  return (
    <div className="bg-obsidian min-h-screen text-white">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute top-0 right-1/2 translate-x-1/2 w-[800px] h-[600px] bg-citrus/5 blur-[120px] rounded-full pointer-events-none" />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-citrus text-[10px] font-black uppercase tracking-[0.3em] mb-6"
          >
          ...
          >
            Deployments of <br /> <span className="text-citrus">High Performance.</span>
          </motion.h1>
      ...
      <section className="bg-white/[0.02] py-32 border-y border-white/5 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <h2 className="font-outfit font-black text-2xl text-white mb-16 uppercase tracking-widest border-l-4 border-citrus pl-6">Archive // <span className="text-gray-500">Declassified Projects</span></h2>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-citrus/10 to-transparent pointer-events-none" />
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 relative z-10">
            {[
              ...
              { value: '21', label: 'Specialized Agents', icon: Zap },
              ...
              <div key={stat.label} className="text-center group-hover:scale-105 transition-transform duration-500">
                <stat.icon className="w-8 h-8 text-citrus mx-auto mb-6 opacity-40" />
                <p className="font-outfit font-black text-5xl text-white mb-2 tracking-tighter">{stat.value}</p>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-500">{stat.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <section className="py-40 relative">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h2 className="font-outfit font-black text-5xl text-white mb-8 tracking-tighter">
            Architect Your <span className="text-citrus">Case Study.</span>
          </h2>
